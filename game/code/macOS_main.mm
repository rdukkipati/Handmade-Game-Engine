...
@interface HandmadeApplicationDelegate : NSObject<NSApplicationDelegate, NSWindowDelegate>
@end
...
- (NSSize)windowWillResize:(NSWindow *)window toSize:(NSSize)frameSize
{
    ...
    f32 WindowMinusContentWidth = (WindowRect.size.width - ContentRect.size.width);
    f32 WindowMinusContentHeight = (WindowRect.size.height - ContentRect.size.height);
    
    f32 NewContentHeight = (9.0f * (frameSize.width - WindowMinusContentWidth)) / 16.0f;
...
#pragma unused(argv)
    ...
    @autoreleasepool
    {
        ...
        NSRect InitialFrame = NSMakeRect((ScreenRect.size.width - WindowWidth) * 0.5f, (ScreenRect.size.height - WindowHeight) * 0.5f, WindowWidth, WindowHeight);
